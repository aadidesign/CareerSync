...
const ApplicationsPage = () => {
  ...
  const getFilteredApplications = () => {
    ...
    return applications.filter((app: ApplicationWithJob) => app.status === activeTab);
  };

  const getStatusCounts = () => {
    if (!applications) return { all: 0 };
    
    const counts: Record<string, number> = { all: applications.length };
    
    applications.forEach((app: ApplicationWithJob) => {
          ...
          <Tabs defaultValue="all" className="w-full" value={activeTab} onValueChange={setActiveTab}>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
              <TabsList className="premium-glass">
                <TabsTrigger value="all">
                  All
                  {statusCounts.all > 0 && <Badge className="ml-2 bg-white/10">{statusCounts.all}</Badge>}
                </TabsTrigger>
            ...
            <TabsContent value={activeTab} className="mt-0">
              {isLoading ? (
                ...
                <AnimatePresence>
                  <div className="grid gap-4">
                    {filteredApplications.map((application: ApplicationWithJob) => (
                      <motion.div
                        key={application.id}
                            ...
                            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                              ...
                              <div className="flex flex-col items-end gap-2">
                                ...
                                <div className="flex flex-col items-end text-sm">
                                  ...
                                  <span className="text-silver-500 text-xs">Last updated: {formatDate(application.updated_at)}</span>
                                </div>
                              </div>
                            </div>
                          </CardContent>
